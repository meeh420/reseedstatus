...
def _read_rid(rid,data,pksize=256,sksize=128):
    ind = _mutnum()
    rid['pkey'] = _getnext(data,pksize,ind)
    rid['skey'] = _getnext(data,sksize,ind)
    rid['cert'] = _getrest(data,ind)
...
def _read_entry(entry,fname):
    ...
    _put_next_mapping(opts,ind,data)
    entry['options'] = opts
    entry['signature'] = _getnext(data,40,ind)
    return True
...
def inspect(hook=_print_hook,netdb_dir=os.path.join(os.environ['HOME'],'.i2p','netDb')):
    ...
    """
    for fname in os.listdir(netdb_dir):
        fname = os.path.join(netdb_dir,fname)
        e = dict()
